...
function buildADF(lead: any, mapping: typeof SOURCE_MAP[string]): string {
  ...
  const fullSource = [mapping.source, mapping.subsource, lead.adgroup ? `Ad Group: ${lead.adgroup}` : null, lead.keyword ? `Keyword: ${lead.keyword}` : null].filter(Boolean).join(' | ')
  const comments = [
  ...
  return `<?xml version="1.0" encoding="UTF-8"?><?adf version="1.0"?><adf><prospect status="new"><id sequence="1" source="${escapeXml(fullSource)}">${Date.now()}</id><requestdate>${requestDate}</requestdate><vehicle interest="${isUsed ? 'Used' : 'New'}" status="available"><year></year><make>GMC</make><model>${escapeXml(lead.trim || mapping.subsource)}</model><trim>${escapeXml(lead.trim || '')}</trim></vehicle><customer><contact primarycontact="1"><name part="first">${escapeXml(lead.firstName || '')}</name><phone type="voice" time="day">${escapeXml(lead.phone || '')}</phone></contact><comments>${escapeXml(comments)}</comments></customer><vendor><vendorname>Larry Spacc GMC</vendorname><contact primarycontact="1"><name part="full">AD COMMAND</name><email>leads@adcommand.one</email></contact></vendor><provider><name part="full">${escapeXml(mapping.source)}</name><service>${escapeXml(mapping.subsource)}</service><url>${escapeXml(lead.landingPage || '')}</url></provider></prospect></adf>`
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!lead.phone || !lead.firstName) {
      return NextResponse.json({ error: 'Missing required fields: firstName, phone' }, { status: 400 })
    }
    ...
    const subject  = [`[${mapping.source}]`, mapping.subsource, lead.keyword ? `"${lead.keyword}"` : null, '— Larry Spacc GMC'].filter(Boolean).join(' ')
    ...
    const { error: dbError } = await supabase.from('leads').insert({
      source: mapping.source, subsource: mapping.subsource,
      campaign: lead.campaign || 'unknown', adgroup: lead.adgroup || 'unknown',
      keyword: lead.keyword || 'unknown', first_name: lead.firstName,
      phone: lead.phone, trim: lead.trim || 'Any', config: lead.config || null,
      notes: lead.notes || null, landing_page: lead.landingPage || null,
      tracking_line: lead.trackingLine || '716-268-1096',
      department: mapping.dept, tcpa_consent: true,
      resend_id: data?.id || null, created_at: new Date().toISOString(),
    })

    if (dbError) console.error('Supabase log error:', dbError)

    return NextResponse.json({
      success: true, message: 'Lead received and forwarded to DealerSocket',
      leadId: data?.id, source: mapping.source, subsource: mapping.subsource,
    })
  } catch (err: any) {
